...
import { GAPS, type GapDef, computeGap } from "@/lib/gaps";
...
function laterDate(pointA: IndicatorPoint, pointB: IndicatorPoint): string {
  return pointA.date > pointB.date ? pointA.date : pointB.date;
}

function rankGap(def: GapDef, pointA: IndicatorPoint, pointB: IndicatorPoint): RankedGap | undefined {
  ...
  return {
    ...
    sourcesText: `${sourceLabel(def.legs[0].source)} · ${sourceLabel(def.legs[1].source)} · ${formatDateAR(laterDate(pointA, pointB))}`,
...
function rankGaps(queries: UseQueryResult<IndicatorSeries>[]): RankedGap[] {
  const ranked: RankedGap[] = [];
  GAPS.forEach((def, index) => {
    const pointA = queries[index * LEGS_PER_GAP]?.data?.points[0];
    const pointB = queries[index * LEGS_PER_GAP + 1]?.data?.points[0];
    if (!pointA || !pointB) {
      return;
    }
    const rankedGap = rankGap(def, pointA, pointB);
    if (rankedGap !== undefined) {
      ranked.push(rankedGap);
    }
  });
  return ranked.sort((first, second) => second.gapMagnitude - first.gapMagnitude);
}

function RankedGapRow({ gap, index }: Readonly<{ gap: RankedGap; index: number }>): ReactElement {
  const strong = index < HIGHLIGHTED_POSITIONS;
  return (
    <Link
      href={`#${gap.id}`}
...
export function GapRankList(): ReactElement {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {ranked.map((gap, index) => (
        <RankedGapRow key={gap.id} gap={gap} index={index} />
      ))}
    </div>
  );
}
